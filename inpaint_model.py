...
class InpaintCAModel(Model):
    ...
    def build_inpaint_net(self, x, mask, reuse=False,
        ...
        with tf.variable_scope(name, reuse=reuse), \
            ...
            #print("conv1:",(x.shape))#(16, 256, 256, 24)
            x = gen_conv(x, 2*cnum, 3, 2, name='conv2_downsample')
            #print("conv2_downsample:",(x.shape))#(16, 128, 128, 48)
            ...
            #print("x:",(x.shape))#(16, 64, 64, 96)
            x1 = gen_conv(x, 4*cnum, 3, rate=2, name='conv7_atrous')
            x2 = gen_conv(tf.concat([x,x1],axis=-1), 4*cnum, 3, rate=4, name='conv8_atrous')
            x3 = gen_conv(tf.concat([x,x1,x2],axis=-1), 4*cnum, 3, rate=8, name='conv9_atrous')
            x4 = gen_conv(tf.concat([x,x1,x2,x3],axis=-1), 4*cnum, 3, rate=16, name='conv10_atrous')
            ...
            x1 = gen_conv(x, 4 * cnum, 3, rate=2, name='xconv7_atrous')
            x2 = gen_conv(tf.concat([x, x1], axis=-1), 4 * cnum, 3, rate=4, name='xonv8_atrous')
            x3 = gen_conv(tf.concat([x, x1, x2], axis=-1), 4 * cnum, 3, rate=8, name='xconv9_atrous')
            x4 = gen_conv(tf.concat([x, x1, x2, x3], axis=-1), 4 * cnum, 3, rate=16, name='xconv10_atrous')
            x = tf.concat([x1, x2, x3, x4], axis=-1)
            x_hallu = x
            # attention branch
